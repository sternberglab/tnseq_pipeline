py_name = 'Linear_Hist_Sites_batch_custom_scale_percentage.py'
### NOTE - Script edited to plot custom y scale for visual 'normalization'
...
bin_size = 5000  # in bp
exp_date = 190202
mode = 'Trans_Sites'  # either 'Coverage' or 'Trans_Sites'
percent = 0.50 # caps the y axis at X% of total reads
...
def readnbin(filename, bin_size):  # main binning function
    ...
    # convert lists to arrays for matplotlib, optional??
    b = np.asarray(b)
    a2 = np.asarray(a2)

    return b, a2  # return the lists. If binning using coverage, also return max(a) here for ytick labels later


### NOTE - Script edited to plot custom y scale (with manually defined % of max value) for visual 'normalization' ###
def plot(filename, bin_size, date, desc, psl, spacer_location):  # main graphing function
    spacer_bin = int(spacer_location/bin_size) + 0.5  # determine which bin the spacer lies in

    # set up figure
    fig, axs = plt.subplots(1, 1, tight_layout=True)
    b, a2 = readnbin(filename, bin_size)  # call readnbin to get arrays for xy axes
    sum_reads = sum(a2) # total number of reads
    max_y = percent * 0.01 * sum_reads
    ...
    # 2 ticks on the y axis, one at 0 and on at max value of y (X percent of total reads)
    # if using coverage, take max(a) from readnbin instead (the 'pre binning' maximum)
    axs.set_yticks([0,max_y])
    axs.set_yticklabels([0,percent])
    ...
    plt.gca().set_ylim(bottom=-0.12*max_y, top=max_y)

    # info text (MOVED TO THE SIDE)
    plt.text(33*bin_scale, 0.8*max_y, "{}-{}\n{}\ngRNA Plasmid = {}\nTotal Reads = {}"
             .format(code, date, desc, psl, sum_reads))

    # save figures
    plt.savefig('plots\\small_scale\\0.5percent\\{}percent{}_{}_{}.svg'.format(percent, date, code, desc), dpi=500)
...
with open('input.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        if filename != 'none':
            ...
            plot(filename, bin_size, exp_date, desc, psl, spacer_location)
        else:
