py_name = 'trans_dist_mmei_v2_batch.py'

# Leo Vo, Feb 2019
# For measuring distance between end of protospacer to TN7 flanks for NGS reads from an MmeI NGS library
# v2 - Reads in Fasta files outputed from Leo's python pipeline,
# which contain 17bp fingerprints that map unique to the genome (no longer reversed)
#
...
os.chdir('C:\\Users\\Leo Vo\\Desktop\\New_NGS_Dec_2019\\zipped_fastq\\fingerprinted\\mapped_reads')
...
excel_name = '{}_transposition_distance_mmei_SINGLE.xlsx'.format(exp_date) ## Change output excel file name here
...
on_target_window = 100 # in bp, will be cut in half and rounded (if not even)
reads_input_filetype = "fasta" # for fingerprinted reads input

#reads in BL21DE3 RefSeq
for record in SeqIO.parse("genome_mg1655.fasta", "fasta"):
...
def dist_find(code, psl, description, filename, direction, refseq, spacer, date, excel, plot, plot_overlap,
              on_target_window):  # main analysis function
    ...
    for record in SeqIO.parse(filename, reads_input_filetype):  # loop through all reads in fastq file
        total += 1
...
with open('input_JM.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        filename = 'none'
        for i in os.listdir('.'):
            if fnmatch.fnmatch(i, "*{}*.{}".format(code, reads_input_filetype)):
                filename = i
                break
        if filename != 'none':
            description = row[6]
            ...
            dist_find(code, psl, description, filename, direction, refseq, spacer, exp_date, excel, plot,
                      plot_overlap, on_target_window)
        else:
